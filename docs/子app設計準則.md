...
若 App 是**獨立匯出的靜態 HTML**（不會載入 `shared.js`，例如報讀工具存到報讀庫的文件），要把 `pickBestZhVoice()` 的邏輯直接內嵌一份到匯出的樣板字串裡；且要留意**已經匯出的舊文件是寫死的快照**，之後修這段邏輯不會回頭套用到舊文件，若有大量既有文件，需另外寫一次性的批次修復工具（對已存的 html 字串做字串比對＋替換，文字內容不受影響），修復完、確認全部既有文件都更新後即可將該工具移除，不需要長期保留。
